...
class ServiceNowClient:
    ...
    def __init__(self):
        ...
        # Auth priority: API key > OAuth token > Basic auth
        if api_key:
...
USERNAME = os.getenv("SERVICENOW_USERNAME") or os.getenv("SNOW_USERNAME", "")
PASSWORD = os.getenv("SERVICENOW_PASSWORD") or os.getenv("SNOW_PASSWORD", "")
_API_KEY = os.getenv("SERVICENOW_API_KEY") or os.getenv("SNOW_API_KEY", "")
_OAUTH_TOKEN = os.getenv("SERVICENOW_OAUTH_TOKEN") or os.getenv("SNOW_OAUTH_TOKEN", "")


def _legacy_request_kwargs() -> dict:
    """Returns auth kwargs for legacy direct requests calls.
    Supports API key, OAuth token, or basic auth from environment variables."""
    base_headers = {"Accept": "application/json", "Content-Type": "application/json"}
    if _API_KEY:
        return {"headers": {**base_headers, "X-sn-apikey": _API_KEY}, "auth": None}
    if _OAUTH_TOKEN:
        return {"headers": {**base_headers, "Authorization": f"Bearer {_OAUTH_TOKEN}"}, "auth": None}
    return {"headers": base_headers, "auth": (USERNAME, PASSWORD)}
...
def list_pending_approvals(
    ...
    # Exclude approvals where the parent document is no longer active
    # (e.g. request closed/cancelled but approval record still shows "requested")
    query_parts.append("sysapproval.active=true")
...
def query_snow_table_sc(table, query="", fields="", limit=100, display_value="false"):
    ...
    try:
        req_kwargs = _legacy_request_kwargs()
        response = requests.get(url, params=params, timeout=30, **req_kwargs)
...
def get_request_status(request_number: str) -> str:
    ...
    try:
        ...
        # Fetch up to 10 matching records (number can be reused after closure) and
        # sort in Python to guarantee we get the newest — avoids relying on
        # ServiceNow's ORDER BY handling through URL-encoded query params.
        result = query_snow_table_sc(
            table,
            query=f"number={request_number}",
            fields="sys_id,number,state,stage,approval,active,sys_created_on,opened_at,opened_by,requested_for,short_description,description,assignment_group,assigned_to,work_notes,comments",
            limit=10,
            display_value="all"
        )
        ...
        def _raw_val(field_data):
            if isinstance(field_data, dict):
                return field_data.get("value", "")
            return field_data or ""

        # Sort newest first: active records float to top, then by sys_created_on DESC
        def _sort_key(r):
            active = _raw_val(r.get("active")) not in ("false", False, "0")
            created = _raw_val(r.get("sys_created_on"))
            return (0 if active else 1, created)

        sorted_records = sorted(result["result"], key=_sort_key, reverse=True)
